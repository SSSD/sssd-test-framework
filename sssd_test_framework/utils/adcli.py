...
class AdcliUtils(MultihostUtility[MultihostHost]):
    ...
    .. note::
       ...
       For methods requiring an authentication, --prompt-password(-W) is a default. Setting krb=True will enable
       kerberos based authentication. The --prompt-password is similar to --stdin-password
    """
    ...
    def join(
        ...
        *,
        ...
        if krb:
            self.host.conn.exec(["kinit", login_user], input=password)
            command = self.host.conn.exec(["adcli", "join", "--verbose", "-C", *args, domain])
        else:
            command = self.host.conn.exec(
                ["adcli", "join", "--prompt-password", "--verbose", f"--login-user={login_user}", *args, domain],
                input=password,
            )
    ...
    def delete_computer(
        ...
        *,
        ...
        if krb:
            ...
            command = self.host.conn.exec(
                ["adcli", "delete-computer", "--promp-password", "--verbose", *args], input=password
            )
    ...
    def show_computer(
        ...
        *,
        ...
        if krb:
            self.host.conn.exec(["kinit", login_user], input=password)
            command = self.host.conn.exec(["adcli", "show-computer", f"--domain={domain}", "--verbose", "-C", *args])
        else:
            command = self.host.conn.exec(
                ["adcli", "show-computer", "--prompt-password", f"--domain={domain}", "--verbose", *args],
                input=password,
            )
