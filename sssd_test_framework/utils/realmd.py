...
class RealmUtils(MultihostUtility[MultihostHost]):
    ...
    .. code-block:: python
        ...
        def test_realm_discover(client: Client, provider: ADProvider):
            r = client.realm.discover([p_domain, "--use-ldaps"])
    ...
    def leave(self, args: list[str] | None = None) -> ProcessResult:
        """
    ...
    def join(
        ...
        *,
        ...
        :param args: Additional arguments, defaults to None
        :type args: list[str] | None, optional
        :param password: Password to run the operation, defaults to None
        :type password: str
        ...
        if krb:
            ...
            command = self.host.conn.exec(["realm", "join", "--verbose", *args, domain])
        else:
            command = self.host.conn.exec(["realm", "join", "--verbose", *args, "-U", user, domain], input=password)

        return command

    def rlist(self, *, args: list[str] | None = None) -> ProcessResult:
        """
