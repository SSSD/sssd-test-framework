...
class IPA(BaseLinuxRole[IPAHost]):
    ...
    @staticmethod
    def ipa_search(role: IPA, command: str, criteria: str | None = None, attr: str = "cn") -> list[str]:
        """
...
class IPAHostGroup(IPAObject):
    ...
    def show(self, attrs: list[str] | None = None) -> dict[str, list[str]] | None:
        ...
        :param attrs: List of attribute names to extract, or None to get all.
    ...
    @classmethod
    def search(cls, role: IPA, criteria: str | None = None) -> list[str]:
        """
        Search for host groups matching criteria.

        :param role: IPA role object.
        :param criteria: Search filter string.
        :return: List of matching HBAC host groups names.
        :rtype: list[str]
        """
        return IPA.ipa_search(role, "hostgroup-find", criteria)
    ...
    def add_member_manager(
        ...
        :param host: Host(s) to add as member manager(s).
        :type host: list[str] | str | None, optional
        :param hostgroup: Host group(s) to add as member manager(s).
        :type hostgroup: list[str] | str | None, optional
        :return: Self.
        :rtype: IPAHostGroup
        """
    ...
    def remove_member_manager(
        ...
        :param host: Host(s) to remove as member manager(s).
        :type host: list[str] | str | None, optional
        :param hostgroup: Host group(s) to remove as member manager(s).
        :type hostgroup: list[str] | str | None, optional
        :return: Self.
        :rtype: IPAHostGroup
        """
...
class IPAHBACService(IPAObject):
    ...
    def show(self, attrs: list[str] | None = None) -> dict[str, list[str]] | None:
        ...
        :param attrs: If set, only requested attributes are returned, defaults to None
        :type attrs: list[str] | None, optional
        :return: Dictionary of HBAC service attributes or None if not found
        :rtype: dict[str, list[str]] | None
        """
    ...
    def search(cls, role: IPA, criteria: str | None = None) -> list[str]:
        ...
        :param role: IPA role object.
        :param criteria: Search filter string.
        :return: List of matching HBAC service names.
        """
        return IPA.ipa_search(role, "hbacsvc-find", criteria)
...
class IPAHBACServiceGroup(IPAObject):
    ...
    def search(cls, role: IPA, criteria: str | None = None) -> list[str]:
        """
        Search for HBAC service group matching criteria.

        :param role: IPA role object.
        :param criteria: Search filter string.
        :return: List of matching HBAC service group names.
        :rtype: list[str] | None
        """
        return IPA.ipa_search(role, "hbacsvcgroup-find", criteria)
...
class IPAHBAC(IPAObject):
    ...
    def search(cls, role: IPA, criteria: str | None = None) -> list[str]:
        ...
        :param role: IPA role object.
        :param criteria: Search filter string.
        :return: List of matching HBAC rules names.
        :rtype: list[str] | None
        """
        return IPA.ipa_search(role, "hbacrule-find", criteria)
    ...
    def remove_members(
        ...
        *,
        ...
        """
        if users:
            users_list = [users] if isinstance(users, str) else users
            for user in users_list:
                self.role.host.conn.exec(["ipa", "hbacrule-remove-user", self.name, f"--users={user}"])

        if hosts:
            hosts_list = [hosts] if isinstance(hosts, str) else hosts
            for host in hosts_list:
                self.role.host.conn.exec(["ipa", "hbacrule-remove-host", self.name, f"--hosts={host}"])

        if services:
            services_list = [services] if isinstance(services, str) else services
            for service in services_list:
                self.role.host.conn.exec(["ipa", "hbacrule-remove-service", self.name, f"--hbacsvcs={service}"])
    ...
    def status(
        ...
        *,
        ...
        services = to_list(get_attr(raw_data, "Services", default=[]))
        service_groups = to_list(get_attr(raw_data, "Service Groups", default=[]))
