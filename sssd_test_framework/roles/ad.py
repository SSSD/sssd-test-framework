...
class ADCertificateAuthority:
    ...
    def request(
        ...
        subject: str,
        key_size: int = 2048,
        machine_keyset: bool = False,
        exportable: bool = True,
    ) -> tuple[str, str, str]:
        """
        Request a certificate from the AD CA.

        :param template: Certificate template name (e.g., "User", "Computer", "WebServer").
        :type template: str
        :param subject: Certificate subject (e.g., "CN=testuser").
        :type subject: str
        :param key_size: RSA key size in bits.
        :type key_size: int
        :param machine_keyset: Whether to use machine key set.
        :type machine_keyset: bool
        :param exportable: Whether the private key is exportable.
        ...
        Subject = "{subject}"
        KeyLength = {key_size}
        MachineKeySet = {"yes" if machine_keyset else "no"}
        Exportable = {"yes" if exportable else "no"}
        RequestType = PKCS10
        KeyUsage = 0xA0
